...
void execute_sum(int requested_index)
{
    ...
    ocl1::device_provider provider = ocl1::device_provider();
    ocl1::device device = provider.select_device(requested_index);
    const ocl1::execution_context execution_context_instance = ocl1::execution_context(device, dimension_config);

    ocl1::program_builder builder = ocl1::program_builder(execution_context_instance);
...
void execute_mult(int requested_index, std::string input_path, std::string output_path)
{
    ...
    ocl1::device_provider provider = ocl1::device_provider();
    ocl1::device device = provider.select_device(requested_index);
    ocl1::execution_context execution_context_instance = ocl1::execution_context(device, dimension_config);
